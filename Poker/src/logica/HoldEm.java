...
public class HoldEm implements GeneralJuego {
    ...
    public HoldEm(ArrayList<Jugador> pJugadores) {
        
        this.CartasVisibles = 5;
        this.CartasPrivadas = 2;
            ...
            for(int j=0; j< Jugadores.size(); j++)
                {
                    ...
                    System.out.println("Largo lista mesa" + Mesa.size());
                    System.out.println("Largo lista jugador" + Jugadores.get(j).getCartasJugador().size());
                    System.out.println(Mesa.get(0).imprimirla());
                    System.out.println(Mesa.get(1).imprimirla());
                    System.out.println(Mesa.get(2).imprimirla());
                    System.out.println(Mesa.get(3).imprimirla());
                    System.out.println(Mesa.get(4).imprimirla());
                    Hand mano=new Hand();
   ...
   public int getGanador(ArrayList<Jugador> Lista,int contador) {
            ...
            for(int i=1; i < Lista.size();i++)
            {
               ...
               if(puntajeGanador==puntajeTemp){
                   ...
                   System.out.println("puntaje1=" +puntajeGanador);
                   System.out.println("puntaje2=" +puntajeTemp);
  ...
  for (int i=0; i< ServidorPrincipal.ListaJ.size(); i++)
       {
           ...
           //Cliente Turno = new Cliente("algo;b;"+String.valueOf(0), ServidorPrincipal.ListaJ.get(i).IpJugador);
       ...
       System.out.println("PRIMERA RONDA DE APUESTAS");
       for (int i = 0; i < ServidorPrincipal.ListaJ.size(); i ++)
       {   
           ServidorApuestas = new Servidor(ServidorPrincipal.ListaJ,ListaApuestas,ApuestaGeneralMain);
           
           System.out.println("Apuesta a insertar primera vez: "+ServidorApuestas.ListaJ.get(i).getApuesta().valor);
           ListaApuestas.add(ServidorApuestas.ListaJ.get(i).getApuesta());
           System.out.println("Inicia lista apuestas primera vez");
           for(int b=0; b < ListaApuestas.size(); b++) /// imprimer lista de apuestas inicial
           {
               System.out.println(ListaApuestas.get(b).jugador.nombre);
               System.out.println(ListaApuestas.get(b).valor);
           }
           System.out.println("Finaliza lista apuestas");
       ...
       {    
            ...
            for (int j=0; j< ServidorApuestas.ListaJ.size(); j++)
            {
                if (ServidorApuestas.ListaJ.get(j).Igualado == false)
                {
                    ...
                    //Cliente Turno = new Cliente("algo;yy;b;"+String.valueOf(0), ServidorPrincipal.ListaJ.get(i).IpJugador);
                    //System.out.println("IP DEL JUGADOR: "+ ServidorPrincipal.ListaJ.get(i).IpJugador);
                    
                    System.out.println("Apuesta a insertar: "+ServidorApuestas.ListaJ.get(j).getApuesta().valor);
                    ListaApuestas.add(ServidorApuestas.ListaJ.get(j).getApuesta());
                    System.out.println("Inicia Apuestas");
                    for(int l=0; l < ListaApuestas.size(); l++)
                    {
                        System.out.println(ListaApuestas.get(l).jugador.nombre);
                        System.out.println(ListaApuestas.get(l).valor);
                    }
                    System.out.println("Finaliza apuestas");
                    ApuestaGeneralMain = ServidorApuestas.ApuestaGeneral;
                    ServidorApuestas.ApuestaGeneral = ApuestaGeneralMain;
       ...
       ServidorApuestas.ApuestaGeneral = 0;
             
       System.out.println("SEGUNDA RONDA DE APUESTAS");
       for (int a = 0; a < ServidorPrincipal.ListaJ.size(); a ++)
       {   
           ServidorApuestas = new Servidor(ServidorPrincipal.ListaJ,ListaApuestas,ApuestaGeneralMain);
           
           ListaApuestas.add(ServidorApuestas.ListaJ.get(a).getApuesta());
           ApuestaGeneralMain = ServidorApuestas.ApuestaGeneral;
           //System.out.println("VALOR ACTUALIZADO:"+ApuestaGeneralMain);
           ServidorApuestas.ApuestaGeneral = ApuestaGeneralMain;
           //System.out.println("Pozo General: "+PozoGeneral.TotalPozo());
       ...
       {    
            ...
            for (int j=0; j< ServidorApuestas.ListaJ.size(); j++)
            {
                if (ServidorApuestas.ListaJ.get(j).Igualado == false)
                {
                    ...
                    ListaApuestas.add(ServidorApuestas.ListaJ.get(j).getApuesta());
                    //System.out.println("Inicia lista apuestas primera vez");
                for(int p=0; p < ListaApuestas.size(); p++)
                {
                    System.out.println(ListaApuestas.get(p).jugador.nombre);
                    System.out.println(ListaApuestas.get(p).valor);
                }
                    System.out.println("FInaliza lista apuestas");
                    ApuestaGeneralMain = ServidorApuestas.ApuestaGeneral;
                    ServidorApuestas.ApuestaGeneral = ApuestaGeneralMain;
       ...
       //***************************************************************
       System.out.println("TERCERA RONDA DE APUESTAS");
       ServidorApuestas.ApuestaGeneral = 0;
       //Cliente MostrarCartasT = new Cliente("algo;yy;m2;"+String.valueOf(PozoGeneral.TotalPozo()), ServidorPrincipal.ListaJ.get(0).IpJugador);
       
       
       for (int b = 0; b < ServidorPrincipal.ListaJ.size(); b ++)
       {   
           ServidorApuestas = new Servidor(ServidorPrincipal.ListaJ,ListaApuestas,ApuestaGeneralMain);
           ListaApuestas.add(ServidorApuestas.ListaJ.get(b).getApuesta());
           ApuestaGeneralMain = ServidorApuestas.ApuestaGeneral;
           //System.out.println("VALOR ACTUALIZADO:"+ApuestaGeneralMain);
           ServidorApuestas.ApuestaGeneral = ApuestaGeneralMain;
           //System.out.println("Pozo General: "+PozoGeneral.TotalPozo());
       ...
       {    
            ...
            for (int j=0; j< ServidorApuestas.ListaJ.size(); j++)
            {
                if (ServidorApuestas.ListaJ.get(j).Igualado == false)
                {
                    ...
                    //System.out.println("Apuesta por jugador: "+ ServidorApuestas.ListaJ.get(j).getApuesta().valor);
                    ListaApuestas.add(ServidorApuestas.ListaJ.get(j).getApuesta());
                    System.out.println("Inicia lista apuestas vez");
                    for(int l=0; l < ListaApuestas.size(); l++)
                    {
                        System.out.println(ListaApuestas.get(l).jugador.nombre);
                        System.out.println(ListaApuestas.get(l).valor);
       ...
       //***************************************************************
       System.out.println("CUARTA RONDA DE APUESTAS");
       ServidorApuestas.ApuestaGeneral = 0;
       //Cliente TurnoC = new Cliente("algo;yy;b;"+String.valueOf(0), ServidorPrincipal.ListaJ.get(0).IpJugador);
       for (int c = 0; c < ServidorPrincipal.ListaJ.size(); c ++)
       {   
           ServidorApuestas = new Servidor(ServidorPrincipal.ListaJ,ListaApuestas,ApuestaGeneralMain);
           ListaApuestas.add(ServidorApuestas.ListaJ.get(c).getApuesta());
           System.out.println("Inicia lista apuestas vez");     //TURNO 
                    for(int s=0; s < ListaApuestas.size(); s++)
                    {
                        System.out.println(ListaApuestas.get(s).jugador.nombre);
                        System.out.println(ListaApuestas.get(s).valor);
                    }
                    System.out.println("Finaliza lista apuestas");
           ApuestaGeneralMain = ServidorApuestas.ApuestaGeneral;
